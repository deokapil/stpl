...
def app():
    ...
    if Analyze:
        st. markdown(
            f"<h4 style='text-align: center;'> {team} </h4>", unsafe_allow_html=True)

        ###########################################################
        # -------->   AVERAGE SCORE AGAINST EACH TEAM      <-------
        ###########################################################
        selected_team_df = new_deliveriesDF[new_deliveriesDF['batting_team'].str.strip(
        ) == team]

        innings_data = selected_team_df.groupby(['match_id',
                                                 'inning',
                                                 'bowling_team'])['total_runs'].sum().reset_index()
        ...
        ###########################################################
        team_over_data = (selected_team_df.groupby('over')[
                          'total_runs'].mean()*6).round().astype(int).reset_index()
...
######################################################################################################################################
        ...
        team_toss_decision = new_matchesDF[new_matchesDF['toss_winner']
                                           == team]['toss_decision']
...
######################################################################################################################################
        ...
        # Top 10 Lowest Runs
        fig = plt.figure(figsize=(12, 10))
        team_runs_over_df = selected_team_df.groupby(['match_id', 'bowling_team', 'inning'])[
            'total_runs'].sum().reset_index().sort_values(by='total_runs', ascending=True)
